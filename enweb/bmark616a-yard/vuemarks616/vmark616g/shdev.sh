...
# run developmet (dev) server:
  ...
  docker run --name sh-dev_vmark616fdev_021 --rm -v "${PWD}:/$(basename `pwd`)" -w "/$(basename `pwd`)" -p $devport:8081  \
      -e     "VIRTUAL_HOST=vmark616devg.198.23.238.244.nip.io" \
      -e "LETSENCRYPT_HOST=vmark616devg.198.23.238.244.nip.io" \
      -e "VIRTUAL_PORT=8081" \
    ...
    # -it  $nodeimg sh -c "npm run serve --host 198.23.238.244 --port 8081"
